...
public class PathfindingTest
{
    ...
    private static void RunHPASimulation(int iterations, (int min, int max) densityRange, (int min, int max) checkPointsRange, (int min, int max) cellularIterationsRange, (int min, int max) heightRange, (int min, int max) widthRange, string name)
    {
        ...
        for (int i = 0; i < iterations; i++)
        {
            ...
            (HPAPath hpaPath1, var timeLevel1) = EffiencyInNodeExploration(tileMap, start, end, 1, RefinePath: false);

            (HPAPath hpaPath2, var timeLevel2) = EffiencyInNodeExploration(tileMap, start, end, 2, RefinePath: false);

            (HPAPath hpaPath3, var timeLevel3) = EffiencyInNodeExploration(tileMap, start, end, 3, RefinePath: false);
    ...
    public static (HPAPath, long) EffiencyInNodeExploration(int[,] tileMap, Vector2Int start, Vector2Int end, int maxLevel, Boolean RefinePath = false)
    {
